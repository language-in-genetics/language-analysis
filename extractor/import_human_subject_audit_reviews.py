#!/usr/bin/env python3
"""
Import Homo sapiens audit review state from the merah SQLite database into PostgreSQL.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import Homo sapiens audit reviews from SQLite into PostgreSQL")
    ...
    if not sqlite_table_exists(sqlite_conn, "human_subject_audit_batches"):
        print(f"SQLite database has no human_subject_audit_batches table: {sqlite_path}")
        sqlite_conn.close()
        return 0
    ...
    try:
        ...
        for batch in sqlite_batches:
            pg_cur.execute(
                ...
                ON CONFLICT (slug) DO UPDATE SET
                    ...
                    ai_human_sample_size = EXCLUDED.ai_human_sample_size,
                    ai_not_human_sample_size = EXCLUDED.ai_not_human_sample_size,
                ...
                (
                    ...
                    batch["seed"],
                    batch["sample_size"],
                    batch["ai_human_sample_size"],
                    batch["ai_not_human_sample_size"],
                    batch["created_at"],
                    batch["created_by"],
                    batch["source_filter"],
                    batch["notes"],
        ...
        sqlite_articles = sqlite_conn.execute(
            ...
            ORDER BY batch_slug, classification_id
            """
        ).fetchall()
        ...
        for row in sqlite_articles:
            ...
            pg_cur.execute(
                ...
                ON CONFLICT (batch_id, human_subject_classification_id) DO UPDATE SET
                    ...
                    ai_about_humans = EXCLUDED.ai_about_humans,
                ...
                (
                    ...
                    row["title"],
                    row["abstract"],
                    nullable_bool(row["ai_about_humans"]),
                    row["ai_evidence"],
                    row["ai_confidence"],
                    row["ai_model"],
            ...
            if row["reviewer_about_humans"] is None:
                pg_cur.execute(
                    """
                    DELETE FROM languageingenetics.human_subject_audit_reviews
            ...
            pg_cur.execute(
                """
                INSERT INTO languageingenetics.human_subject_audit_reviews (
                    sample_article_id,
                    reviewer_about_humans,
                    reviewer_username,
                    review_notes,
                ...
                ON CONFLICT (sample_article_id) DO UPDATE SET
                    reviewer_about_humans = EXCLUDED.reviewer_about_humans,
                ...
                (
                    sample_article_id,
                    nullable_bool(row["reviewer_about_humans"]),
    ...
    print(
        f"Imported {imported_batches} Homo sapiens audit batches, {imported_articles} articles, "
